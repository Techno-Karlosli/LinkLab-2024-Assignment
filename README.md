...
这些信息在内存中用 C++ 结构体表示（定义在 [`include/fle.hpp`](include/fle.hpp) 中）：
...
> [!IMPORTANT]
> FLE 文件的格式和 `FLEObject` 中的表示，虽然大部分内容是对应的，但其中节数据的表示是不同的。FLE 文件更强调可读性，而 `FLEObject` 则更类似标准的 ELF 格式。具体来说：
>
...
而 `.rodata` 节的内容会被存储在 `FLESection` 结构中：
